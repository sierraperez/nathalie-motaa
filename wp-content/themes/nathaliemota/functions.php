...
// Função para filtrar as fotos via AJAX HEAD
...
function enqueue_photo_filter_scripts() {
    ...
    wp_enqueue_script(
        'photo-filter-script',
        get_template_directory_uri() . '/js/photo-filter.js', // Atualize o caminho conforme necessário
        array('jquery'),
        '1.0',
        true
    );

    // Passa variáveis para o script JavaScript
    wp_localize_script('photo-filter-script', 'ajax_object', array(
        'ajaxurl' => admin_url('admin-ajax.php'), // URL para requisições AJAX
        'nonce'   => wp_create_nonce('nathalie_mota_nonce'), // Nonce para validação de segurança
    ));
}
add_action('wp_enqueue_scripts', 'enqueue_photo_filter_scripts');
...
function filter_photos() {
    ...
    wp_die(); // Finaliza corretamente a requisição AJAX
}
